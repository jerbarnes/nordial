...
def import_data(json_file, tokenizer):
    # pull in current tweets and get frequency of uni, bi- and trigrams
    sents = {"bokmål":[], "nynorsk":[], "dialectal": [], "mixed": [] }
    with open(json_file) as infile:
        ...
        for example in data:
            ...
            for sent in doc.sentences:
                ...
                sents[label].append(tokenized)
    # remove any possible duplicates
    for label in sents.keys():
        sents[label] = list(set(sents[label]))
    return sents
...
if __name__ == "__main__":
    ...
    train_sents = import_data("dialect_train.json", nn_tokenizer)
    di_uni, di_bi, di_tri = get_most_common_features(train_sents["dialectal"])
    b_uni, b_bi, b_tri = get_most_common_features(train_sents["bokmål"])
    n_uni, n_bi, n_tri = get_most_common_features(train_sents["nynorsk"])
    ...
    # chi squared
    uni_f2 = chi_squared(di_uni, n_uni)
    bi_f2 = chi_squared(di_bi, n_bi)
    tri_f2 = chi_squared(di_tri, n_tri)

    chi_terms = combine_grams([uni_f2, bi_f2, tri_f2])
    print("Chi_squred(dialect, nynorsk)")
    for term, metric in chi_terms.most_common(30):
        print("-- {0} - {1:.1f}".format(term, metric))
